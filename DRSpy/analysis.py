...
class Analysis():
    ...
    def get_avg_delay(self, edges=["C", "D", "U"], fit=False, filename="delay_avg.png"):
        ...
        for edge in edges:
            wavg = [[], []] # POS, WAVG
            with click.progressbar(self.distances, label=f"---> Calculating weighted averages for edge {edge} ") as distances:
    ...
    def get_p2p_spectra(self, dist, filename="P2P.png", fit=False):
                ...
                if distance >= dist[0] and distance <= dist[-1]:
                    ...
                    for edge in self.positions:
                        ...
                        try:
                            ...
                            asym_data[edge].append([distance, as1, as2, as3,cc[0], cc[1]])
                            ####
        ...
        for i, j in enumerate(asym.keys()):
            vis.add_plot(axx, asym[j][0], asym[j][1], xlabel="Distance [cm]", ylabel=pp[0], legend=True, grid=True, fmt="o", label=f"{j}", alpha=0.8)
        vis.save(figg, "asym.png")
        figg, axx = vis.create_figure()
        for i, j in enumerate(asym.keys()):
            vis.add_plot(axx, asym[j][0], asym[j][2], xlabel="Distance [cm]", ylabel=pp[1], legend=True, grid=True, fmt="o", label=f"{j}", alpha=0.8)
    ...
    def det_delay_spectra(self,dist, filename="delay.png"):    
        fig, ax = vis.create_figure()
        with click.progressbar(self.distances, label=f"---> Calculating delay specturm ") as distances:
            for distance in distances: 
                if distance >= dist[0] and distance <= dist[-1]:
                    for edge in self.positions:
                        try:
                            ...
                            vis.add_plot(ax, delay[0], delay[1], xlabel="Delay [ns]", ylabel="Counts", title=f"Delay. Range {dist[0]} - {dist[-1]} cm", legend=True, grid=True, fmt="-", label=f"{distance} {edge}", alpha=0.8)
