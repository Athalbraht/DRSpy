...
@click.option(
    ...
    "--db",
    "fdb",
    help="Path to spectra database",
    type=str,
    default="drs_spectra.csv",
    show_default=True,
)
@click.option(
    "-x",
    "--dbxml",
    "fdbxml",
    help="Path to waveforms database",
    type=str,
    default="drs_waveforms.csv",
    show_default=True,
)
@click.option(
    "-m",
...
@click.option(
    ...
    "--auto",
    "fauto",
    help="Recognize file automatically in specified folder",
    is_flag=True,
    default=False,
)
@click.option(
    ...
    type=click.Choice(["xml", "PtP", "delay"]),
    default="PtP",
    show_default=True,
)
@click.option(
    "-t", "--tag", "ftag", help="Add <tag> to data headers", default="", type=str
)
...
@click.option(
    "--exec", "fexec", help="Execute without print()", is_flag=True, default=False
)
...
@click.option(
    "--live", "flive", help="Enable live preview", is_flag=True, default=False
)
...
def run(ctx):
    ...
    for i, j in enumerate(cx0):
        ...
        cx_x = np.array(tm)
        cx_pp0, _ = curve_fit(
            landau, cx_x[0][::1][:-15], cx_x[1][::1][:-15], maxfev=6500
        )
        cx_pp1, _ = curve_fit(
            landau, cx_x[0][::1][:-15], cx_x[2][::1][:-15], maxfev=6500
        )
        cD.append(nconv(j))
        cH0.append(cx_pp0[0])
        cH1.append(cx_pp1[0])
        ...
        ctx.obj.data.plot.line(
            "Channel", cx0[i], ax=ax, alpha=0.7, c="red", markersize=1
        )
        ctx.obj.data.plot.line(
            "Channel", cx1[i], ax=ax, alpha=0.7, c="green", markersize=1
        )
        ...
        if j.replace("_C", "_U") in ctx.obj.data.columns:
            ux_x = (
                ctx.obj.data[
                    ["Channel", cx0[i].replace("_C", "_U"), cx1[i].replace("_C", "_U")]
                ]
                .dropna()
                .to_numpy()
                .T
            ...
            ux_x = np.array(ux_x)
            ux_pp0, _ = curve_fit(
                landau, ux_x[0][::1][:-15], ux_x[1][::1][:-15], maxfev=6500
            )
            ux_pp1, _ = curve_fit(
                landau, ux_x[0][::1][:-15], ux_x[2][::1][:-15], maxfev=6500
            )
            ctx.obj.data.plot.line(
                "Channel",
                cx0[i].replace("_C", "_U"),
        ...
        if j.replace("_C", "_D") in ctx.obj.data.columns:
            dx_x = (
                ctx.obj.data[
                    ["Channel", cx0[i].replace("_C", "_D"), cx1[i].replace("_C", "_D")]
                ]
                .dropna()
                .to_numpy()
                .T
            ...
            if dx_x.shape != (3, 0):
                dx_x = np.array(dx_x)
                dx_pp0, _ = curve_fit(
                    landau, dx_x[0][::1][:-15], dx_x[1][::1][:-15], maxfev=6500
                )
                dx_pp1, _ = curve_fit(
                    landau, dx_x[0][::1][:-15], dx_x[2][::1][:-15], maxfev=6500
                )
                ctx.obj.data.plot.line(
                    "Channel",
                    cx0[i].replace("_C", "_D"),
